...
public class AvalonTileSolidHook : ModHook
{
	...
	private static void On_WorldGen_GetDesiredStalactiteStyle(On_WorldGen.orig_GetDesiredStalagtiteStyle orig, int x, int j, out bool fail, out int desiredStyle, out int height, out int y)
	{
		...
		switch (fail)
		{
			case true when desiredStyle == ModContent.TileType<PolloStone>():
				...
				{
					Main.tile[x, i].TileType = (ushort)ModContent.TileType<PolloStalac>();
				}
				break;

			case false when Main.tile[x, j].TileType == ModContent.TileType<PolloStalac>():
				for (var i = y; i < y + height; i++)
				{
	...
	{
		Main.tileSolid[ModContent.TileType<Snotquicksand>()] = false;
		orig.Invoke(self, sw);
	}
	...
	{
		orig.Invoke();
		Main.tileSolid[ModContent.TileType<Snotquicksand>()] = true;
	}
