...
namespace Xenon.Common.Templates;
...
public abstract class FlailTemplate : ModProjectile
{
	...
	public override void AI()
	{
		...
		int launchTimeLimit = LaunchTimeLimit;  // How much time the projectile can go before retracting (speed and shootTimer will set the flail's range)
		float launchSpeed = LaunchSpeed; // How fast the projectile can move
		float maxLaunchLength = MaxLaunchLength; // How far the projectile's chain can stretch before being forced to retract when in launched state
		float retractAcceleration = RetractAcceleration; // How quickly the projectile will accelerate back towards the player while retracting
		float maxRetractSpeed = MaxRetractSpeed; // The max speed the projectile will have while retracting
		float forcedRetractAcceleration = ForcedRetractAcceleration; // How quickly the projectile will accelerate back towards the player while being forced to retract
		float maxForcedRetractSpeed = MaxForcedRetractSpeed; // The max speed the projectile will have while being forced to retract
		float unusedRetractAcceleration = 1f;
		float unusedMaxRetractSpeed = 14f;
		int unusedChainLength = 60;
		int defaultHitCooldown = DefaultHitCooldown; // How often your flail hits when resting on the ground, or retracting
		int spinHitCooldown = SpinHitCooldown; // How often your flail hits when spinning
		int movingHitCooldown = MovingHitCooldown; // How often your flail hits when moving
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		if (CurrentAIState == AIState.LaunchingForward)
		{
			...
			{
				float trailRemap = (Projectile.oldPos.Length - k) / (float)Projectile.oldPos.Length;
