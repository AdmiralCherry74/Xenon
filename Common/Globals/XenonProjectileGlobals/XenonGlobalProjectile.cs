...
internal class XenonGlobalProjectile : GlobalProjectile
{
    ...
    {
        if (XenonMod.AvalonContentEnabled && XenonMod.TheConfectionRebirthContentEnabled) return;
    ...
    public override void OnKill(Projectile projectile, int timeLeft)
    {
        if (projectile.type == ProjectileID.WorldGlobe)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && player.InModBiome<Autumn>())
            {
                int rand = Main.rand.Next(XenonWorld.AutumnBGCount);
                if (rand == XenonWorld.AutumnBG)
                    rand++;
                if (rand > XenonWorld.AutumnBGCount - 1)
                    rand = 0;
                XenonWorld.AutumnBG = rand;
                if (!Main.gameMenu)
                {
                    XenonWorld.AutumnBGFlash = 1f;
                }
