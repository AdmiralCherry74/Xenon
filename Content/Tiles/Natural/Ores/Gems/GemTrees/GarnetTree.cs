...
public class GarnetTree : AvalonModTree
{
	public override string TexturePath => "Xenon/Content/Tiles/Natural/Ores/Gems/GemTrees/GarnetTree";

	public static GrowTreeSettings GemTree_Garnet = new GrowTreeSettings
	{
		GroundTest = GemTreeGroundTest,
		WallTest = GemTreeWallTest,
		TreeHeightMax = 12,
		TreeHeightMin = 7,
		TreeTileType = (ushort)ModContent.TileType<GarnetTree>(),
		TreeTopPaddingNeeded = 4,
		SaplingTileType = (ushort)ModContent.TileType<GarnetSapling>()
	};

	public static TreePaintingSettings GemTreeGarnet = new TreePaintingSettings
	{
	...
	{
		int dustType = ((!WorldGen.genRand.NextBool(10)) ? 1 : ModContent.DustType<GarnetGemDust>());
	...
	public override IEnumerable<Item> GetItemDrops(int i, int j)
	{
		...
		int dropItemStack = 1;
		SetGemTreeDrops(ModContent.ItemType<Garnet>(), ModContent.ItemType<GarnetGemcorn>(), tileCache, ref dropItem, ref secondaryItem);
		if (dropItem == 3)
		{
	...
	private static void EmitGarnetLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
	{
		...
		{
			Gore.NewGoreDirect(new EntitySource_Misc(""), vector, Utils.RandomVector2(Main.rand, -num3, num3), ModContent.GoreType<GarnetGemLeaves>(), 0.7f + Main.rand.NextFloat() * 0.6f).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].TileColor;
		}
	}
				...
				case 22:
					{
						...
						EmitGarnetLeaves(x, y, grassPosX, floorY3);
				...
				case 44:
					{
						...
						EmitGarnetLeaves(x, y, num21 + num2, floorY2);
				...
				case 66:
					{
						...
						EmitGarnetLeaves(x, y, num17 + num18, floorY);
