...
public class CorrosionPot : ModTile
{
        ...
        if (!WorldGen.gen && Main.netMode != 1)
        {
            if (WorldGen.genRand.NextBool(15))
            {
                if (j < Main.worldSurface)
                {
                    ...
                    if (num6 == 1)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.ShinePotion, 1, false, 0, false);
                    }
                    if (num6 == 2)
                    {
                        if (Main.hardMode && WorldGen.genRand.NextBool(2))
                    ...
                    if (num6 == 3)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.SwiftnessPotion, 1, false, 0, false);
                    }
                    if (num6 == 4)
                    {
                ...
                else if (j < Main.rockLayer)
                {
                    ...
                    if (num7 == 0)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.RegenerationPotion, 1, false, 0, false);
                    }
                    if (num7 == 1)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.ShinePotion, 1, false, 0, false);
                    ...
                    if (num7 == 2)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.NightOwlPotion, 1, false, 0, false);
                    }
                    if (num7 == 3)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.SwiftnessPotion, 1, false, 0, false);
                    ...
                    if (num7 == 6)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.HunterPotion, 1, false, 0, false);
                    }
                    if (num7 == 7)
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ItemID.MiningPotion, 1, false, 0, false);
            ...
            else
            {
                ...
                else if (num10 == 2)
                {
                    ...
                    {
                        Item.NewItem(WorldGen.GetItemSource_FromTileBreak(i, j), i * 16, j * 16, 16, 16, ModContent.ItemType<Xenon.Content.Items.Placeable.Tile.Corrosion.CorrosionTorch>(), torchStack); //Corrosion Torch
                    }
                ...
                else
                {
                    ...
                    SpecialUtilities.DropCoinsProperly(num11, i * 16, j * 16);
                }
            }
        }
    }
}
public class CorrosionPotHook : ModHook
{
	...
	{
		if (Main.tile[x, y + 1].TileType == ModContent.TileType<Gutstone>() ||
				Main.tile[x, y + 1].TileType == ModContent.TileType<Gutsand>() ||
				Main.tile[x, y + 1].TileType == ModContent.TileType<CorrosionGrass>())
		{
			style = 0;
			type = (ushort)ModContent.TileType<CorrosionPot>();
		}
