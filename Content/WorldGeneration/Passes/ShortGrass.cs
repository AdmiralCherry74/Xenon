...
namespace Xenon.Content.WorldGeneration.Passes
{
    internal class ShortGrass : GenPass
    {
        ...
        {
            int grass = ModContent.TileType<CorrosionShortGrass>();
            for (int num244 = 0; num244 < Main.maxTilesX; num244++)
            {
                ...
                {
                    if (Main.tile[num244, num245].TileType == ModContent.TileType<CorrosionGrass>() && Main.tile[num244, num245].HasUnactuatedTile)
                    {
                        if (!Main.tile[num244, num245 - 1].HasTile && !Main.tile[num244, num245].IsHalfBlock && Main.tile[num244, num245].Slope == SlopeType.Solid)
                        {
                            Tile t = Main.tile[num244, num245 - 1];
                            ...
                            else if (WorldGen.genRand.NextBool(35) || Main.tile[num244, num245 - 1].WallType == ModContent.WallType<CorrosionGrassWall>())
                            {
