...
public abstract class MarineSandBall : ModProjectile
{
    public override string Texture => "Xenon/Content/Projectiles/FallingTiles/MarineSandBall";
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        if (p.X >= 0 && p.X < Main.maxTilesX && p.Y >= 0 && p.Y < Main.maxTilesY)
        {
            ...
            {
                Item.NewItem(WorldGen.GetItemSource_FromTileBreak(p.X, p.Y), p.X * 16, p.Y * 16, 16, 16, ModContent.ItemType<MarineSandBlock>());
            ...
            if (!t.HasTile && t.TileType != TileID.MinecartTrack)
            {
                ...
                WorldGen.PlaceTile(p.X, p.Y, ModContent.TileType<MarineSand>(), forced: true);
...
public class MarineSandBallFallingProjectile : MarineSandBall
{
    public override void SetStaticDefaults()
    {
        base.SetStaticDefaults();
        ProjectileID.Sets.FallingBlockTileItem[Type] = new(ModContent.TileType<MarineSand>(), ModContent.ItemType<MarineSandBlock>());
    }
    ...
    public override void AI()
    {
        ...
        {
            int i = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<Dusts.MarineSandDust>(), 0f, Projectile.velocity.Y * 0.5f);
...
public class MarineSandSandgunProjectile : MarineSandBall
{
    public override void SetStaticDefaults()
    {
        base.SetStaticDefaults();
        ProjectileID.Sets.FallingBlockTileItem[Type] = new(ModContent.TileType<MarineSand>());
    }
    public override void SetDefaults()
    {
    ...
    public override void AI()
    {
        if (Main.rand.NextBool(2))
        {
            int i = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<MarineSandDust>(), 0f, Projectile.velocity.Y * 0.5f);
            Main.dust[i].velocity.X *= 0.2f;
        }
        Projectile.ai[1]++;
