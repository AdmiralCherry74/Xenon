...
namespace Xenon.Content.Projectiles.Tools.CletaminationSpray;
...
public class CorrosionSpray : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        var dustType = ModContent.DustType<CorrosionSolutionDust>();

        if (Projectile.owner == Main.myPlayer)
        {
            var size = largerSize ? 3 : 2;
            var point = Projectile.Center.ToTileCoordinates();
            ConversionHelper.ConvertToCorrosion(point.X, point.Y, size);
        }
