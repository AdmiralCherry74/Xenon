...
namespace Xenon.Content.NPCs.TortoiseAI
{
    public class Aclugic : ModNPC
    {
        ...
        {
            if (spawnInfo.Player.InModBiome<Corrosion>() && !spawnInfo.Player.InPillarZone())
            {
                return 0.75f;
            }
            else if (spawnInfo.Player.InModBiome<CorrosionUnderground>() && !spawnInfo.Player.InPillarZone())
            {
                return 0.75f;
            }
            return 0;
