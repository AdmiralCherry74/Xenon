...
public class TropicGrassSeeds : ModItem
{
    ...
    public override bool? UseItem(Player player)
    {
        ...
        if (tile.HasTile && player.IsInTileInteractionRange(Player.tileTargetX, Player.tileTargetY, TileReachCheckSettings.Simple))
        {
            ...
            {
                Main.tile[Player.tileTargetX, Player.tileTargetY].TileType = (ushort)ModContent.TileType<BeachGrass>();
                WorldGen.SquareTileFrame(Player.tileTargetX, Player.tileTargetY, true);
                SoundEngine.PlaySound(SoundID.Dig, player.Center);
                return true;
            }
            if (tile.TileType == ModContent.TileType<MarineSand>())
            {
                Main.tile[Player.tileTargetX, Player.tileTargetY].TileType = (ushort)ModContent.TileType<MarineGrass>();
                WorldGen.SquareTileFrame(Player.tileTargetX, Player.tileTargetY, true);
                SoundEngine.PlaySound(SoundID.Dig, player.Center);
                return true;
            }
